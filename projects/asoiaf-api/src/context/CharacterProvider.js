...
class CharacterProvider extends React.Component {
  ...
  getHouses = pageNum => {
    //?page=2
    axios
      .get(`https://www.anapioficeandfire.com/api/houses?page=${pageNum}`)
      .then(res => {
        // console.log(res)
  ...
  formattedSearch = () => {
    ...
    const firstName =
      nameInDB[0][0].toUpperCase() + nameInDB[0].slice(1).toLowerCase();
    const lastName =
      nameInDB[1][0].toUpperCase() + nameInDB[1].slice(1).toLowerCase();
  ...
  handleSubmit = e => {
    ...
    axios
      .get(`https://api.got.show/api/show/characters/${formatted}`)
      .then(res => {
        this.setState(
          {
